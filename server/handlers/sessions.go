...
func registerSessionHandler(implantConn *core.ImplantConnection, data []byte) *CIPHERC2pb.Envelope {
	if implantConn == nil {
		return nil
	}
	register := &CIPHERC2pb.Register{}
...
func auditLogSession(session *core.Session, register *CIPHERC2pb.Register) {
...
func tunnelDataHandler(implantConn *core.ImplantConnection, data []byte) *CIPHERC2pb.Envelope {
	...
	tunnelData := &CIPHERC2pb.TunnelData{}
...
func tunnelCloseHandler(implantConn *core.ImplantConnection, data []byte) *CIPHERC2pb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)
	if session == nil {
...
func pingHandler(implantConn *core.ImplantConnection, data []byte) *CIPHERC2pb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)
	if session == nil {
...
func socksDataHandler(implantConn *core.ImplantConnection, data []byte) *CIPHERC2pb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)
	if session == nil {
	...
	tunnelHandlerMutex.Lock()
	defer tunnelHandlerMutex.Unlock()
	socksData := &CIPHERC2pb.SocksData{}
...
func createReverseTunnelHandler(implantConn *core.ImplantConnection, data []byte) *CIPHERC2pb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)

	req := &CIPHERC2pb.TunnelData{}
	...
	if err != nil {
		tunnelClose, _ := proto.Marshal(&CIPHERC2pb.TunnelData{
			Closed:   true,
			TunnelID: req.TunnelID,
		})
		implantConn.Send <- &CIPHERC2pb.Envelope{
			Type: CIPHERC2pb.MsgTunnelClose,
			Data: tunnelClose,
		}
		cancelContext()
		return nil
	...
	if tunnelDataCache.Len(tunnel.ID) > 3 {
		data, err := proto.Marshal(&CIPHERC2pb.TunnelData{
...
func RTunnelDataHandler(tunnelData *CIPHERC2pb.TunnelData, tunnel *rtunnels.RTunnel, connection *core.ImplantConnection) {
