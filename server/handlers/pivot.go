...
func pivotPeerEnvelopeHandler(implantConn *core.ImplantConnection, data []byte) *CIPHERC2pb.Envelope {
	pivotLog.Debugf("received pivot peer envelope ...")
	peerEnvelope := &CIPHERC2pb.PivotPeerEnvelope{}
	...
	var resp *CIPHERC2pb.Envelope
	switch peerEnvelope.Type {
	case CIPHERC2pb.MsgPivotServerKeyExchange:
		resp = serverKeyExchange(implantConn, peerEnvelope)
	case CIPHERC2pb.MsgPivotSessionEnvelope:
...
func sessionEnvelopeHandler(implantConn *core.ImplantConnection, peerEnvelope *CIPHERC2pb.PivotPeerEnvelope) *CIPHERC2pb.Envelope {
...
func handlePivotEnvelope(pivot *core.Pivot, envelope *CIPHERC2pb.Envelope) {
	...
	} else if envelope.Type == CIPHERC2pb.MsgPivotServerPing {
...
func pivotPeerFailureHandler(implantConn *core.ImplantConnection, data []byte) *CIPHERC2pb.Envelope {
	peerFailure := &CIPHERC2pb.PivotPeerFailure{}
...
// ------------------------
func serverKeyExchange(implantConn *core.ImplantConnection, peerEnvelope *CIPHERC2pb.PivotPeerEnvelope) *CIPHERC2pb.Envelope {
	...
	serverKeyEx := &CIPHERC2pb.PivotServerKeyExchange{}
	...
	keyExRespEnvelope := MustMarshal(&CIPHERC2pb.Envelope{
		Type: CIPHERC2pb.MsgPivotServerKeyExchange,
		Data: MustMarshal(&CIPHERC2pb.PivotServerKeyExchange{
	...
	peerEnvelopeData, _ := proto.Marshal(&CIPHERC2pb.PivotPeerEnvelope{
	...
	pivotSession.Start()
	return &CIPHERC2pb.Envelope{
		Type: CIPHERC2pb.MsgPivotPeerEnvelope,
...
func peersToString(remoteAddr string, peerEnvelope *CIPHERC2pb.PivotPeerEnvelope) string {
