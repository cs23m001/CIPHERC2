...
import (
	...
	"github.com/cs23m001/CIPHERC2/server/netstack"
...
func StartWGListener(port uint16, netstackPort uint16, keyExchangeListenPort uint16) (net.Listener, *device.Device, *bytes.Buffer, error) {
	...
	if err != nil {
		wgLog.Errorf("Failed to setup up wg CIPHERC2 listener: %v", err)
		return nil, nil, nil, err
	}
	wgLog.Printf("Successfully setup up wg CIPHERC2 listener")
	go acceptWGCIPHERC2Connections(listener)
...
func acceptWGCIPHERC2Connections(ln net.Listener) {
	for {
		conn, err := ln.Accept()
		if err != nil {
			if errType, ok := err.(*net.OpError); ok && errType.Op == "accept" {
		...
		go handleWGCIPHERC2Connection(conn)
	}
}

func handleWGCIPHERC2Connection(conn net.Conn) {
...
func socketWGWriteEnvelope(connection net.Conn, envelope *CIPHERC2pb.Envelope) error {
...
func socketWGReadEnvelope(connection net.Conn) (*CIPHERC2pb.Envelope, error) {
