...
func StartMutualTLSListener(bindIface string, port uint16) (net.Listener, error) {
	...
	go acceptCIPHERC2Connections(ln)
	return ln, nil
}

func acceptCIPHERC2Connections(ln net.Listener) {
	for {
		conn, err := ln.Accept()
		if err != nil {
		...
		go handleCIPHERC2Connection(conn)
	}
}

func handleCIPHERC2Connection(conn net.Conn) {
...
func socketWriteEnvelope(connection net.Conn, envelope *CIPHERC2pb.Envelope) error {
...
func socketReadEnvelope(connection net.Conn) (*CIPHERC2pb.Envelope, error) {
