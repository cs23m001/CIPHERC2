...
import (
	...
	CIPHERC2Handlers "github.com/cs23m001/CIPHERC2/server/handlers"
...
// HTTPSession - Holds data related to a CIPHERC2 c2 session
...
// CIPHERC2HTTPC2 - Holds refs to all the C2 objects
type CIPHERC2HTTPC2 struct {
...
func (s *CIPHERC2HTTPC2) getServerHeader() string {
...
func (s *CIPHERC2HTTPC2) getCookieName(c2ConfigName string) string {
...
func StartHTTPListener(req *clientpb.HTTPListenerReq) (*CIPHERC2HTTPC2, error) {
	httpLog.Infof("Starting https listener on '%s'", req.Host)
	server := &CIPHERC2HTTPC2{
		ServerConf: req,
		HTTPSessions: &HTTPSessions{
			active: map[string]*HTTPSession{},
			mutex:  &sync.RWMutex{},
...
func (s *CIPHERC2HTTPC2) loadServerHTTPC2Configs() *clientpb.HTTPC2Configs {
...
func (s *CIPHERC2HTTPC2) router() *mux.Router {
...
func (s *CIPHERC2HTTPC2) noCacheHeader(resp http.ResponseWriter) {
	resp.Header().Add("Cache-Control", "no-store, no-cache, must-revalidate")
}

// This filters requests that do not have a valid nonce
func (s *CIPHERC2HTTPC2) filterNonce(req *http.Request, rm *mux.RouteMatch) bool {
...
func (s *CIPHERC2HTTPC2) DefaultRespHeaders(next http.Handler) http.Handler {
	return http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
		var (
...
func (s *CIPHERC2HTTPC2) websiteContentHandler(resp http.ResponseWriter, req *http.Request) error {
...
func (s *CIPHERC2HTTPC2) defaultHandler(resp http.ResponseWriter, req *http.Request) {
...
// [ HTTP Handlers ] ---------------------------------------------------------------
func (s *CIPHERC2HTTPC2) mainHandler(resp http.ResponseWriter, req *http.Request) {
	extension := strings.TrimLeft(path.Ext(req.URL.Path), ".")
...
func (s *CIPHERC2HTTPC2) startSessionHandler(resp http.ResponseWriter, req *http.Request) {
	...
	sessionInit := &CIPHERC2pb.HTTPSessionInit{}
...
func (s *CIPHERC2HTTPC2) sessionHandler(resp http.ResponseWriter, req *http.Request) {
	...
	handlers := CIPHERC2Handlers.GetHandlers()
	if envelope.ID != 0 {
...
func (s *CIPHERC2HTTPC2) pollHandler(resp http.ResponseWriter, req *http.Request) {
...
func (s *CIPHERC2HTTPC2) readReqBody(httpSession *HTTPSession, resp http.ResponseWriter, req *http.Request) ([]byte, error) {
	nonce, _ := getNonceFromURL(req.URL)
	_, encoder, err := encoders.EncoderFromNonce(nonce)
	if err != nil {
		httpLog.Warnf("Request specified an invalid encoder (%d)", nonce)
...
func (s *CIPHERC2HTTPC2) getServerPollTimeout() time.Duration {
...
func (s *CIPHERC2HTTPC2) closeHandler(resp http.ResponseWriter, req *http.Request) {
...
// stagerHandler - Serves the CIPHERC2 shellcode to the stager requesting it
func (s *CIPHERC2HTTPC2) stagerHandler(resp http.ResponseWriter, req *http.Request) {
	nonce, _ := getNonceFromURL(req.URL)
	...
	if nonce != 0 {
		...
		if build.Stage {
			...
			s.noCacheHeader(resp)
			resp.Write(payload)
			httpLog.Infof("Serving CIPHERC2 shellcode (size %d) %s to %s", len(payload), resourceID.Name, getRemoteAddr(req))
			resp.WriteHeader(http.StatusOK)
		}
	}
	s.defaultHandler(resp, req)
...
func (s *CIPHERC2HTTPC2) getHTTPSession(req *http.Request) *HTTPSession {
	for _, cookie := range req.Cookies() {
