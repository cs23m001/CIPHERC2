...
var (
	builderLog = log.NamedLogger("builder", "CIPHERC2")
)
...
func NewBuilder(config *clientpb.Builder, m *sync.Mutex, rpc rpcpb.CIPHERC2RPCClient, ln *grpc.ClientConn) *Builder {
	return &Builder{
...
func (b *Builder) handleBuildEvent(event *clientpb.Event) {
	...
	if extConfig.Config.TemplateName != "CIPHERC2" {
	...
	case clientpb.OutputFormat_EXECUTABLE:
		b.mutex.Lock()
		fPath, err = generate.CIPHERC2Executable(extConfig.Build.Name, extConfig.Build, extConfig.Config, httpC2Config.ImplantConfig)
		b.mutex.Unlock()
	case clientpb.OutputFormat_SHARED_LIB:
		b.mutex.Lock()
		fPath, err = generate.CIPHERC2SharedLibrary(extConfig.Build.Name, extConfig.Build, extConfig.Config, httpC2Config.ImplantConfig)
		b.mutex.Unlock()
	case clientpb.OutputFormat_SHELLCODE:
		b.mutex.Lock()
		fPath, err = generate.CIPHERC2Shellcode(extConfig.Build.Name, extConfig.Build, extConfig.Config, httpC2Config.ImplantConfig)
		b.mutex.Unlock()
	default:
	...
	if err != nil {
		builderLog.Errorf("Failed to generate CIPHERC2: %s", err)
		b.rpc.BuilderTrigger(context.Background(), &clientpb.Event{
			EventType: consts.ExternalBuildFailedEvent,
			Data:      []byte(fmt.Sprintf("%s:%s", implantBuildID, err.Error())),
		})
	...
	if err != nil {
		builderLog.Errorf("Failed to read generated CIPHERC2: %s", err)
		b.rpc.BuilderTrigger(context.Background(), &clientpb.Event{
			EventType: consts.ExternalBuildFailedEvent,
			Data:      []byte(fmt.Sprintf("%s:%s", implantBuildID, err.Error())),
		})
