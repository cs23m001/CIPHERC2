...
// Request - Sends a protobuf request to the active CIPHERC2 and returns the response
func (s *Session) Request(msgType uint32, timeout time.Duration, data []byte) ([]byte, error) {
	resp := make(chan *CIPHERC2pb.Envelope)
	reqID := EnvelopeID()
	...
	select {
	case s.Connection.Send <- &CIPHERC2pb.Envelope{
		ID:   reqID,
		Type: msgType,
		Data: data,
	}:
	...
	var respEnvelope *CIPHERC2pb.Envelope
	select {
...
// sessions - Manages the CIPHERC2s, provides atomic access
...
// Add - Add a CIPHERC2 to the hive (atomically)
...
// Remove - Remove a CIPHERC2 from the hive (atomically)
