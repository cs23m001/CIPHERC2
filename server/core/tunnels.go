...
// Tunnel  - Essentially just a mapping between a specific client and CIPHERC2
...
type Tunnel struct {
	...
	FromImplant         chan *CIPHERC2pb.TunnelData
	FromImplantSequence uint64

	Client rpcpb.CIPHERC2RPC_TunnelDataServer
...
func NewTunnel(id uint64, sessionID string) *Tunnel {
	return &Tunnel{
		...
		FromImplant: make(chan *CIPHERC2pb.TunnelData),
...
func (t *Tunnel) SendDataFromImplant(tunnelData *CIPHERC2pb.TunnelData) {
...
func (t *tunnels) Close(tunnelID uint64) error {
	...
	tunnelClose, err := proto.Marshal(&CIPHERC2pb.TunnelData{
		TunnelID:  tunnel.ID,
	...
	data, err := proto.Marshal(&CIPHERC2pb.Envelope{
