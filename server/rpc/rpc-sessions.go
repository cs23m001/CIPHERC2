...
func (rpc *Server) KillSession(ctx context.Context, kill *CIPHERC2pb.KillReq) (*commonpb.Empty, error) {
	...
	session.Request(CIPHERC2pb.MsgNumber(kill), timeout, data)
	return &commonpb.Empty{}, nil
}

// OpenSession - Instruct beacon to open a new session on next checkin
func (rpc *Server) OpenSession(ctx context.Context, openSession *CIPHERC2pb.OpenSession) (*CIPHERC2pb.OpenSession, error) {
	resp := &CIPHERC2pb.OpenSession{Response: &commonpb.Response{}}
...
func (rpc *Server) CloseSession(ctx context.Context, closeSession *CIPHERC2pb.CloseSession) (*commonpb.Empty, error) {
	...
	go func() {
		select {
		case session.Connection.Send <- &CIPHERC2pb.Envelope{Type: CIPHERC2pb.MsgCloseSession}:
