...
func (rpc *Server) Kill(ctx context.Context, kill *CIPHERC2pb.KillReq) (*commonpb.Empty, error) {
	var (
...
func (rpc *Server) killSession(kill *CIPHERC2pb.KillReq, session *core.Session) (*commonpb.Empty, error) {
	...
	go session.Request(CIPHERC2pb.MsgNumber(kill), timeout, data)
	core.Sessions.Remove(session.ID)
	return &commonpb.Empty{}, nil
}

func (rpc *Server) killBeacon(kill *CIPHERC2pb.KillReq, beacon *models.Beacon) (*commonpb.Empty, error) {
	resp := &commonpb.Empty{}
	...
	task, err := beacon.Task(&CIPHERC2pb.Envelope{
		Type: CIPHERC2pb.MsgKillSessionReq,
		Data: reqData,
	})
	if err != nil {
		return nil, err
