...
var (
	...
	toImplantCache = dataCache{mutex: &sync.RWMutex{}, cache: map[uint64]map[uint64]*CIPHERC2pb.TunnelData{}}

	// SessionID->Tunnels[TunnelID]->Tunnel->Cache
	fromImplantCache = dataCache{mutex: &sync.RWMutex{}, cache: map[uint64]map[uint64]*CIPHERC2pb.TunnelData{}}
)
...
func (s *Server) CreateTunnel(ctx context.Context, req *CIPHERC2pb.Tunnel) (*CIPHERC2pb.Tunnel, error) {
...
func (s *Server) CloseTunnel(ctx context.Context, req *CIPHERC2pb.Tunnel) (*commonpb.Empty, error) {
...
func (s *Server) TunnelData(stream rpcpb.CIPHERC2RPC_TunnelDataServer) error {
	for {
		...
		if tunnel.Client == nil {
			...
			tunnel.Client.Send(&CIPHERC2pb.TunnelData{
				TunnelID:  tunnel.ID,
			...
			go func() {
				...
				data, _ := proto.Marshal(&CIPHERC2pb.TunnelData{
