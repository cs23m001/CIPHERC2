...
import (
	...
	protobufs "github.com/cs23m001/CIPHERC2/protobuf"
...
func SetupGoPath(goPathSrc string, includeDNS bool) error {
	...
	// CIPHERC2 PB
	CIPHERC2pbGoSrc, err := protobufs.FS.ReadFile("CIPHERC2pb/CIPHERC2.pb.go")
	if err != nil {
		setupLog.Info("Static asset not found: CIPHERC2.pb.go")
		return err
	}
	CIPHERC2pbConstSrc, err := protobufs.FS.ReadFile("CIPHERC2pb/constants.go")
	if err != nil {
		setupLog.Info("Static asset not found: constants.go")
		return err
	}
	CIPHERC2pbGoSrc = xorPBRawBytes(CIPHERC2pbGoSrc)
	CIPHERC2pbGoSrc = stripCIPHERC2pb(CIPHERC2pbGoSrc)
	CIPHERC2pbDir := filepath.Join(goPathSrc, "github.com", "bishopfox", "CIPHERC2", "protobuf", "CIPHERC2pb")
	os.MkdirAll(CIPHERC2pbDir, 0700)
	os.WriteFile(filepath.Join(CIPHERC2pbDir, "CIPHERC2.pb.go"), CIPHERC2pbGoSrc, 0600)
	os.WriteFile(filepath.Join(CIPHERC2pbDir, "constants.go"), CIPHERC2pbConstSrc, 0600)
	...
	commonpbDir := filepath.Join(goPathSrc, "github.com", "bishopfox", "CIPHERC2", "protobuf", "commonpb")
	...
	if includeDNS {
		...
		dnspbDir := filepath.Join(goPathSrc, "github.com", "bishopfox", "CIPHERC2", "protobuf", "dnspb")
...
func stripCIPHERC2pb(src []byte) []byte {
	out := src
...
func xorPBRawBytes(src []byte) []byte {
	var (
		fileAst                     *ast.File
		err                         error
		CIPHERC2pbVarName           = "file_CIPHERC2pb_CIPHERC2_proto_rawDesc"
		CIPHERC2PbProtoInitFuncName = "file_CIPHERC2pb_CIPHERC2_proto_init"
	...
	ast.Inspect(fileAst, func(n ast.Node) bool {
		...
		case *ast.FuncDecl:
			...
			// that calls the xor function on the file_CIPHERC2pb_CIPHERC2_proto_rawDesc object
			if node.Name.Name == CIPHERC2PbProtoInitFuncName {
				newStmt := &ast.AssignStmt{
					Lhs: []ast.Expr{
						ast.NewIdent(CIPHERC2pbVarName),
					},
					Tok: token.ASSIGN,
					Rhs: []ast.Expr{
						&ast.CallExpr{
