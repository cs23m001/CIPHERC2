...
import (
	...
	"github.com/cs23m001/CIPHERC2/implant"
	...
	"github.com/cs23m001/CIPHERC2/server/encoders"
	"github.com/cs23m001/CIPHERC2/server/gogo"
	"github.com/cs23m001/CIPHERC2/server/log"
	"github.com/cs23m001/CIPHERC2/util"
	utilEncoders "github.com/cs23m001/CIPHERC2/util/encoders"
...
const (
	...
	// CIPHERC2CC64EnvVar - Environment variable that can specify the 64 bit mingw path
	CIPHERC2CC64EnvVar = "CIPHERC2_CC_64"
	// CIPHERC2CC32EnvVar - Environment variable that can specify the 32 bit mingw path
	CIPHERC2CC32EnvVar = "CIPHERC2_CC_32"

	// CIPHERC2CXX64EnvVar - Environment variable that can specify the 64 bit mingw path
	CIPHERC2CXX64EnvVar = "CIPHERC2_CXX_64"
	// CIPHERC2CXX32EnvVar - Environment variable that can specify the 32 bit mingw path
	CIPHERC2CXX32EnvVar = "CIPHERC2_CXX_32"

	// *** Platform Specific ***

	// CIPHERC2PlatformCC64EnvVar - Environment variable that can specify the 64 bit mingw path
	CIPHERC2PlatformCC64EnvVar = "CIPHERC2_%s_CC_64"
	// CIPHERC2PlatformCC32EnvVar - Environment variable that can specify the 32 bit mingw path
	CIPHERC2PlatformCC32EnvVar = "CIPHERC2_%s_CC_32"
	// CIPHERC2PlatformCXX64EnvVar - Environment variable that can specify the 64 bit mingw path
	CIPHERC2PlatformCXX64EnvVar = "CIPHERC2_%s_CXX_64"
	// CIPHERC2PlatformCXX32EnvVar - Environment variable that can specify the 32 bit mingw path
	CIPHERC2PlatformCXX32EnvVar = "CIPHERC2_%s_CXX_32"
)

// GetCIPHERC2sDir - Get the binary directory
func GetCIPHERC2sDir() string {
	appDir := assets.GetRootAppDir()
	CIPHERC2sDir := filepath.Join(appDir, CIPHERC2sDirName)
	if _, err := os.Stat(CIPHERC2sDir); os.IsNotExist(err) {
		buildLog.Debugf("Creating bin directory: %s", CIPHERC2sDir)
		err = os.MkdirAll(CIPHERC2sDir, 0700)
		if err != nil {
			buildLog.Fatal(err)
		}
	}
...
// -----------------------
// CIPHERC2 Generation Code
// -----------------------

// CIPHERC2Shellcode - Generates a CIPHERC2 shellcode using Donut
func CIPHERC2Shellcode(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	...
	pkgPath, err := renderCIPHERC2GoCode(name, build, config, goConfig, pbC2Implant)
	if err != nil {
		return "", err
	}
...
// CIPHERC2SharedLibrary - Generates a CIPHERC2 shared library (DLL/dylib/so) binary
func CIPHERC2SharedLibrary(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	appDir := assets.GetRootAppDir()
...
// CIPHERC2Executable - Generates a CIPHERC2 executable binary
func CIPHERC2Executable(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	appDir := assets.GetRootAppDir()
...
func renderCIPHERC2GoCode(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, goConfig *gogo.GoConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	...
	buildLog.Debugf("Generating new CIPHERC2 binary '%s'", name)
	pbC2Implant = models.RandomizeImplantConfig(pbC2Implant, config.GOOS, config.GOARCH)
	CIPHERC2sDir := GetCIPHERC2sDir() // ~/.CIPHERC2/CIPHERC2s
	projectGoPathDir := filepath.Join(CIPHERC2sDir, config.GOOS, config.GOARCH, filepath.Base(name))
	if _, err := os.Stat(projectGoPathDir); os.IsNotExist(err) {
		os.MkdirAll(projectGoPathDir, 0700)
	}
	goConfig.ProjectDir = projectGoPathDir

	// binDir - ~/.CIPHERC2/CIPHERC2s/<os>/<arch>/<name>/bin
	binDir := filepath.Join(projectGoPathDir, "bin")
	os.MkdirAll(binDir, 0700)

	// srcDir - ~/.CIPHERC2/CIPHERC2s/<os>/<arch>/<name>/src
	...
	CIPHERC2PkgDir := filepath.Join(srcDir, "github.com", "bishopfox", "CIPHERC2") // "main"
	err = os.MkdirAll(CIPHERC2PkgDir, 0700)
	if err != nil {
		return "", nil
	}
	...
	err = fs.WalkDir(implant.FS, ".", func(fsPath string, f fs.DirEntry, err error) error {
		...
		CIPHERC2GoCodeRaw, err := implant.FS.ReadFile(fsPath)
		if err != nil {
			buildLog.Errorf("Failed to read %s: %s", fsPath, err)
			return nil
		}
		CIPHERC2GoCode := string(CIPHERC2GoCodeRaw)

		// Skip dllmain files for anything non windows
		if f.Name() == "CIPHERC2.c" || f.Name() == "CIPHERC2.h" {
			if !config.IsSharedLib && !config.IsShellcode {
				return nil
			}
		}

		var CIPHERC2CodePath string
		if f.Name() == "CIPHERC2.go" || f.Name() == "CIPHERC2.c" || f.Name() == "CIPHERC2.h" {
			CIPHERC2CodePath = filepath.Join(CIPHERC2PkgDir, f.Name())
		} else {
			CIPHERC2CodePath = filepath.Join(CIPHERC2PkgDir, "implant", fsPath)
		}
		dirPath := filepath.Dir(CIPHERC2CodePath)
		...
		fCIPHERC2, err := os.Create(CIPHERC2CodePath)
		if err != nil {
			return err
		}
		if !util.Contains([]string{".go", ".c", ".h"}, path.Ext(f.Name())) {
			...
			_, err = fCIPHERC2.Write(CIPHERC2GoCodeRaw)
			return err
		}
		...
		buildLog.Debugf("[render] %s -> %s", f.Name(), CIPHERC2CodePath)

		CIPHERC2Code := template.New("CIPHERC2")
		CIPHERC2Code, err = CIPHERC2Code.Funcs(template.FuncMap{
			"GenerateUserAgent": func() string {
				return pbC2Implant.UserAgent
			},
		}).Parse(CIPHERC2GoCode)
		if err != nil {
			buildLog.Errorf("Template parsing error %s", err)
			return err
		}
		err = CIPHERC2Code.Execute(buf, struct {
			Name                string
		...
		err = canaryTemplate.Execute(fCIPHERC2, canaryGenerator)
		if err != nil {
	...
	renderNativeEncoderAssets(build, config, CIPHERC2PkgDir)
	if config.TrafficEncodersEnabled {
		renderTrafficEncoderAssets(build, config, CIPHERC2PkgDir)
	}

	// Render GoMod
	buildLog.Info("Rendering go.mod file ...")
	goModPath := filepath.Join(CIPHERC2PkgDir, "go.mod")
	err = os.WriteFile(goModPath, []byte(implant.GoMod), 0600)
	if err != nil {
		return "", err
	}
	goSumPath := filepath.Join(CIPHERC2PkgDir, "go.sum")
	...
	err = fs.WalkDir(implant.Vendor, ".", func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			return os.MkdirAll(filepath.Join(CIPHERC2PkgDir, path), 0700)
		}
		...
		return os.WriteFile(filepath.Join(CIPHERC2PkgDir, path), contents, 0600)
	})
	if err != nil {
	...
	return CIPHERC2PkgDir, nil
}

// renderTrafficEncoderAssets - Copies and compresses any enabled WASM traffic encoders
func renderTrafficEncoderAssets(_ *clientpb.ImplantBuild, config *clientpb.ImplantConfig, CIPHERC2PkgDir string) {
	buildLog.Infof("Rendering traffic encoder assets ...")
	encoderAssetsPath := filepath.Join(CIPHERC2PkgDir, "implant", "CIPHERC2", "encoders", "assets")
...
func renderNativeEncoderAssets(_ *clientpb.ImplantBuild, _ *clientpb.ImplantConfig, CIPHERC2PkgDir string) {
...
func GenerateConfig(name string, implantConfig *clientpb.ImplantConfig) (*clientpb.ImplantBuild, error) {
	...
	CIPHERC2Cert, CIPHERC2Key, err := certs.MtlsC2ImplantGenerateECCCertificate(name)
	if err != nil {
		return nil, err
	}
	...
	if models.IsC2Enabled([]string{"mtls"}, implantConfig.C2) {
		...
		build.MtlsCert = string(CIPHERC2Cert)
		build.MtlsKey = string(CIPHERC2Key)
	}
...
func getCrossCompilersFromEnv(targetGoos string, targetGoarch string) (string, string) {
	...
	if targetGoarch == "amd64" {
		if os.Getenv(fmt.Sprintf(CIPHERC2PlatformCC64EnvVar, TARGET_GOOS)) != "" {
			cc = os.Getenv(fmt.Sprintf(CIPHERC2PlatformCC64EnvVar, TARGET_GOOS))
		}
		if cc == "" {
			cc = os.Getenv(CIPHERC2CC64EnvVar)
		}
		if os.Getenv(fmt.Sprintf(CIPHERC2PlatformCXX64EnvVar, TARGET_GOOS)) != "" {
			cc = os.Getenv(fmt.Sprintf(CIPHERC2PlatformCXX64EnvVar, TARGET_GOOS))
		}
		if cxx == "" {
			cxx = os.Getenv(CIPHERC2CXX64EnvVar)
		}
	}
	if targetGoarch == "386" {
		cc = os.Getenv(CIPHERC2CC32EnvVar)
		if os.Getenv(fmt.Sprintf(CIPHERC2PlatformCC32EnvVar, TARGET_GOOS)) != "" {
			cc = os.Getenv(fmt.Sprintf(CIPHERC2PlatformCC32EnvVar, TARGET_GOOS))
		}
		cxx = os.Getenv(CIPHERC2CXX64EnvVar)
		if os.Getenv(fmt.Sprintf(CIPHERC2PlatformCXX32EnvVar, TARGET_GOOS)) != "" {
			cc = os.Getenv(fmt.Sprintf(CIPHERC2PlatformCXX32EnvVar, TARGET_GOOS))
		}
	}
	return cc, cxx
}
