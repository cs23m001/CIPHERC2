...
if go test -tags=server,$TAGS ./util/minisign ; then
    ...
    cat ~/.CIPHERC2/logs/CIPHERC2.log
    exit 1
fi

## Implant

# implant / CIPHERC2 / extension
if go test ./implant/CIPHERC2/extension ; then
    :
else
    exit 1
fi

# implant / CIPHERC2 / transports / dnsclient
if go test ./implant/CIPHERC2/transports/dnsclient ; then
    :
else
    exit 1
fi
