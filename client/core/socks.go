...
func (f *socksProxy) Start(tcpProxy *TcpProxy) error {
	...
	for {
		...
		rpcSocks, err := tcpProxy.Rpc.CreateSocks(context.Background(), &CIPHERC2pb.Socks{
		...
		go connect(connection, proxy, &CIPHERC2pb.SocksData{
...
func connect(conn net.Conn, stream rpcpb.CIPHERC2RPC_SocksProxyClient, frame *CIPHERC2pb.SocksData) {
