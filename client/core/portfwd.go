...
// ChannelProxy binds the CIPHERC2 Tunnel to a net.Conn object
...
func (p *ChannelProxy) dialImplant(ctx context.Context) (*TunnelIO, error) {
	...
	rpcTunnel, err := p.Rpc.CreateTunnel(ctx, &CIPHERC2pb.Tunnel{
	...
	portfwdResp, err := p.Rpc.Portfwd(ctx, &CIPHERC2pb.PortfwdReq{
		Request: &commonpb.Request{
			SessionID: p.Session.ID,
		},
		Host:     p.Host(),
		Port:     p.Port(),
		Protocol: CIPHERC2pb.PortFwdProtoTCP,
		TunnelID: tunnel.ID,
	})
	if err != nil {
		return nil, err
	...
	if portfwdResp.Response != nil && portfwdResp.Response.Err != "" {
		p.Rpc.CloseTunnel(ctx, &CIPHERC2pb.Tunnel{
			TunnelID:  tunnel.ID,
			SessionID: p.Session.ID,
		})
