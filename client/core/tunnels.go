...
type tunnels struct {
	...
	mutex       *sync.RWMutex
	streamMutex *sync.Mutex
	stream      rpcpb.CIPHERC2RPC_TunnelDataClient
}

func (t *tunnels) SetStream(stream rpcpb.CIPHERC2RPC_TunnelDataClient) {
	t.streamMutex.Lock()
...
func (t *tunnels) send(tunnelData *CIPHERC2pb.TunnelData) error {
	t.streamMutex.Lock()
	defer t.streamMutex.Unlock()
...
func (t *tunnels) Start(tunnelID uint64, sessionID string) *TunnelIO {
	...
	go func(tunnel *TunnelIO) {
		...
		for data := range tunnel.Send {
			...
			err := t.send(&CIPHERC2pb.TunnelData{
				TunnelID:  tunnel.ID,
