...
func consoleCmd(con *console.CIPHERC2Client) *cobra.Command {
	consoleCmd := &cobra.Command{
		Use:   "console",
		Short: "Start the CIPHERC2 client console",
	}
...
func consoleRunnerCmd(con *console.CIPHERC2Client, run bool) (pre, post func(cmd *cobra.Command, args []string) error) {
	...
	pre = func(_ *cobra.Command, _ []string) error {
		...
		var rpc rpcpb.CIPHERC2RPCClient
		var err error
		...
		return console.StartClient(con, rpc, command.ServerCommands(con, nil), command.CIPHERC2Commands(con), run)
	}
