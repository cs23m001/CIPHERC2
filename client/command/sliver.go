...
// CIPHERC2Commands returns all commands bound to the implant menu.
func CIPHERC2Commands(con *client.CIPHERC2Client) console.Commands {
	CIPHERC2Commands := func() *cobra.Command {
		CIPHERC2 := &cobra.Command{
		...
		bind := makeBind(CIPHERC2, con)

		// [ Core ]
		bind(consts.CIPHERC2CoreHelpGroup,
			reconfig.Commands,
			// sessions.Commands,
			sessions.CIPHERC2Commands,
			kill.Commands,
			// use.Commands,
			tasks.Commands,
			pivots.Commands,
		...
		for _, manifest := range aliasManifests {
			_, err := alias.LoadAlias(manifest, CIPHERC2, con)
			if err != nil {
		...
		for _, manifest := range extensionManifests {
			...
			for _, ext := range mext.ExtCommand {
				extensions.ExtensionRegisterCommand(ext, CIPHERC2, con)
			}
		}
		...
		CIPHERC2.InitDefaultHelpCmd()
		CIPHERC2.SetHelpCommandGroupID(consts.CIPHERC2CoreHelpGroup)
