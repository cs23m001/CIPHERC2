...
// AliasesLoadCmd - Locally load a alias into the CIPHERC2 shell.
func AliasesLoadCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	dirPath := args[0]
...
// LoadAlias - Load an alias into the CIPHERC2 shell from a given directory.
func LoadAlias(manifestPath string, cmd *cobra.Command, con *console.CIPHERC2Client) (*AliasManifest, error) {
	...
	longHelpMsg += "https://github.com/BishopFox/CIPHERC2/wiki/Aliases-&-Extensions#aliases-command-parsing"
	...
	if aliasManifest.IsAssembly {
		...
		f.BoolP("in-process", "i", false, "Run in the current CIPHERC2 process")
...
func runAliasCommand(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	if aliasManifest.IsAssembly {
		...
		executeAssemblyResp, err := con.Rpc.ExecuteAssembly(context.Background(), &CIPHERC2pb.ExecuteAssemblyReq{
		...
		msg := fmt.Sprintf("Executing %s %s ...", cmd.Name(), extArgsStr)
		con.SpinUntil(msg, ctrl)
		spawnDllResp, err := con.Rpc.SpawnDll(context.Background(), &CIPHERC2pb.InvokeSpawnDllReq{
			Request:     con.ActiveTarget.Request(cmd),
			Args:        args,
			Data:        binData,
		...
		msg := fmt.Sprintf("Executing %s %s ...", cmd.Name(), extArgsStr)
		con.SpinUntil(msg, ctrl)
		sideloadResp, err := con.Rpc.Sideload(context.Background(), &CIPHERC2pb.SideloadReq{
			Request:     con.ActiveTarget.Request(cmd),
			Args:        args,
			Data:        binData,
...
func PrintSpawnDLLOutput(cmdName string, schema *packages.OutputSchema, spawnDllResp *CIPHERC2pb.SpawnDll, outFilePath *os.File, con *console.CIPHERC2Client) {
	var result string
...
func PrintSideloadOutput(cmdName string, schema *packages.OutputSchema, sideloadResp *CIPHERC2pb.Sideload, outFilePath *os.File, con *console.CIPHERC2Client) {
	var result string
...
func PrintAssemblyOutput(cmdName string, schema *packages.OutputSchema, execAsmResp *CIPHERC2pb.ExecuteAssembly, outFilePath *os.File, con *console.CIPHERC2Client) {
	var result string
