...
func WasmCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	execWasmReq := &CIPHERC2pb.ExecWasmExtensionReq{
...
func isRegistered(name string, cmd *cobra.Command, con *console.CIPHERC2Client) bool {
	...
	defer cancel()
	loaded, err := con.Rpc.ListWasmExtensions(grpcCtx, &CIPHERC2pb.ListWasmExtensionsReq{
		Request: con.ActiveTarget.Request(cmd),
	})
	if err != nil {
		return false
...
func idOf(con *console.CIPHERC2Client) string {
...
func runNonInteractive(execWasmReq *CIPHERC2pb.ExecWasmExtensionReq, con *console.CIPHERC2Client) {
...
func runInteractive(cmd *cobra.Command, execWasmReq *CIPHERC2pb.ExecWasmExtensionReq, con *console.CIPHERC2Client) {
	...
	rpcTunnel, err := con.Rpc.CreateTunnel(ctxTunnel, &CIPHERC2pb.Tunnel{
	...
	if wasmExt.Response != nil && wasmExt.Response.Err != "" {
		...
		_, err = con.Rpc.CloseTunnel(context.Background(), &CIPHERC2pb.Tunnel{
			TunnelID:  tunnel.ID,
			SessionID: session.ID,
		})
		if err != nil {
...
func registerWasmExtension(wasmFilePath string, cmd *cobra.Command, con *console.CIPHERC2Client) error {
	grpcCtx, cancel := con.GrpcContext(cmd)
	defer cancel()
	...
	_, err = con.Rpc.RegisterWasmExtension(grpcCtx, &CIPHERC2pb.RegisterWasmExtensionReq{
		Request: con.ActiveTarget.Request(cmd),
		Name:    filepath.Base(wasmFilePath),
		WasmGz:  data,
	})
...
func WasmLsCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}
