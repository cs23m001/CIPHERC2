...
func PsCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func PrintPS(os string, ps *CIPHERC2pb.Ps, interactive bool, flags *pflag.FlagSet, con *console.CIPHERC2Client) {
...
func findKnownSecurityProducts(ps *CIPHERC2pb.Ps) []string {
...
func procRow(tw table.Writer, proc *commonpb.Process, cmdLine bool, con *console.CIPHERC2Client) table.Row {
	session, beacon := con.ActiveTarget.GetInteractive()
...
func GetPIDByName(cmd *cobra.Command, name string, con *console.CIPHERC2Client) int {
	ps, err := con.Rpc.Ps(context.Background(), &CIPHERC2pb.PsReq{
		Request: con.ActiveTarget.Request(cmd),
	})
	if err != nil {
