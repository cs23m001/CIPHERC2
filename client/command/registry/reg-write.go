...
func RegWriteCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	switch valType {
	case CIPHERC2pb.RegistryTypeBinary:
		var (
			v   []byte
			err error
		)
		...
		binaryValue = v
	case CIPHERC2pb.RegistryTypeDWORD:
		...
		dwordValue = uint32(v)
	case CIPHERC2pb.RegistryTypeQWORD:
		...
		qwordValue = v
	case CIPHERC2pb.RegistryTypeString:
		stringValue = value
	default:
		con.PrintErrorf("Invalid type")
		return
	}
	regWrite, err := con.Rpc.RegistryWrite(context.Background(), &CIPHERC2pb.RegistryWriteReq{
...
func PrintRegWrite(regWrite *CIPHERC2pb.RegistryWrite, con *console.CIPHERC2Client) {
