...
func getType(t string) (uint32, error) {
	...
	switch t {
	case "binary":
		res = CIPHERC2pb.RegistryTypeBinary
	case "dword":
		res = CIPHERC2pb.RegistryTypeDWORD
	case "qword":
		res = CIPHERC2pb.RegistryTypeQWORD
	case "string":
		res = CIPHERC2pb.RegistryTypeString
	default:
...
func RegReadCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	var (
		finalPath string
		key       string
	)
	...
	regRead, err := con.Rpc.RegistryRead(context.Background(), &CIPHERC2pb.RegistryReadReq{
		Hive:     hive,
		Path:     finalPath,
		Key:      key,
		Hostname: hostname,
...
func PrintRegRead(regRead *CIPHERC2pb.RegistryRead, con *console.CIPHERC2Client) {
...
func writeHiveDump(data []byte, encoder string, fileName string, saveLoot bool, lootName string, lootType string, lootFileName string, con *console.CIPHERC2Client) {
	var rawData []byte
	var err error
...
func RegReadHiveCommand(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}
	...
	hiveDump, err := con.Rpc.RegistryReadHive(context.Background(), &CIPHERC2pb.RegistryReadHiveReq{
		RootHive:      rootHive,
