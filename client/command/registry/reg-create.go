...
func RegCreateKeyCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	createKey, err := con.Rpc.RegistryCreateKey(context.Background(), &CIPHERC2pb.RegistryCreateKeyReq{
		Hive:     hive,
		Path:     finalPath,
		Key:      key,
		Hostname: hostname,
...
func PrintCreateKey(createKey *CIPHERC2pb.RegistryCreateKey, regPath string, key string, con *console.CIPHERC2Client) {
