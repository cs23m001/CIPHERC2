...
func ArmoryCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func PrintArmoryPackages(aliases []*alias.AliasManifest, exts []*extensions.ExtensionManifest, con *console.CIPHERC2Client) {
	...
	for _, pkg := range entries {
		...
		if extensions.CmdExists(pkg.CommandName, CIPHERC2Menu.Command) {
			color = console.Green
		}
		if con.Settings.SmallTermWidth+urlMargin < width {
...
func PrintArmoryBundles(bundles []*ArmoryBundle, con *console.CIPHERC2Client) {
	tw := table.NewWriter()
	tw.SetStyle(settings.GetTableStyle(con))
