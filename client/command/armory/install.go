...
func ArmoryInstallCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func installBundle(bundle *ArmoryBundle, armoryPK string, forceInstallation bool, clientConfig ArmoryHTTPConfig, con *console.CIPHERC2Client) {
	installList := []string{}
...
func installPackageByName(name, armoryPK string, forceInstallation, promptToOverwrite bool, clientConfig ArmoryHTTPConfig, con *console.CIPHERC2Client) error {
	pendingPackages := make(map[string]string)
...
func installAliasPackage(entry *pkgCacheEntry, promptToOverwrite bool, clientConfig ArmoryHTTPConfig, con *console.CIPHERC2Client) error {
	if entry == nil {
	...
	tmpFile, err := os.CreateTemp("", "CIPHERC2-armory-")
	if err != nil {
		return err
	}
...
func installExtensionPackage(entry *pkgCacheEntry, promptToOverwrite bool, clientConfig ArmoryHTTPConfig, con *console.CIPHERC2Client) error {
	if entry == nil {
		return errors.New("package not found")
	}
	repoURL, err := url.Parse(entry.RepoURL)
