...
func SelectBeacon(con *console.CIPHERC2Client) (*clientpb.Beacon, error) {
...
func GetBeacon(con *console.CIPHERC2Client, beaconID string) (*clientpb.Beacon, error) {
	grpcCtx, cancel := con.GrpcContext(nil)
	defer cancel()
	beacons, err := con.Rpc.GetBeacons(grpcCtx, &commonpb.Empty{})
	if err != nil {
...
func GetBeacons(con *console.CIPHERC2Client) (*clientpb.Beacons, error) {
	grpcCtx, cancel := con.GrpcContext(nil)
	defer cancel()
	beacons, err := con.Rpc.GetBeacons(grpcCtx, &commonpb.Empty{})
	if err != nil {
