...
func CursedElectronCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func avadaKedavraElectron(electronExe string, session *clientpb.Session, cmd *cobra.Command, con *console.CIPHERC2Client, cargs []string) *core.CursedProcess {
	...
	if electronProcess != nil {
		...
		con.PrintWarnf("CIPHERC2 will need to kill and restart the process in order to perform code injection.\n")
...
func checkElectronPath(electronExe string, session *clientpb.Session, cmd *cobra.Command, con *console.CIPHERC2Client) (bool, error) {
...
func checkElectronProcess(electronExe string, session *clientpb.Session, cmd *cobra.Command, con *console.CIPHERC2Client) (*commonpb.Process, error) {
...
func startCursedElectronProcess(electronExe string, session *clientpb.Session, cmd *cobra.Command, con *console.CIPHERC2Client, cargs []string) (*core.CursedProcess, error) {
	...
	electronExec, err := con.Rpc.Execute(context.Background(), &CIPHERC2pb.ExecuteReq{
		Request: con.ActiveTarget.Request(cmd),
		Path:    electronExe,
		Args:    args,
		Output:  false,
