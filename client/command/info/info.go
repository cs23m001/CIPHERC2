...
func InfoCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	var err error
...
func PIDCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func UIDCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}
...
func GIDCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}
...
func WhoamiCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}
	...
	if isWin {
		cto, err := con.Rpc.CurrentTokenOwner(context.Background(), &CIPHERC2pb.CurrentTokenOwnerReq{
...
func PrintTokenOwner(cto *CIPHERC2pb.CurrentTokenOwner, con *console.CIPHERC2Client) {
