...
func JobsCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func PrintJobs(jobs map[uint32]*clientpb.Job, con *console.CIPHERC2Client) {
	tw := table.NewWriter()
...
func JobsIDCompleter(con *console.CIPHERC2Client) carapace.Action {
...
func jobKill(jobID uint32, con *console.CIPHERC2Client) {
...
func killAllJobs(con *console.CIPHERC2Client) {
	jobs, err := con.Rpc.GetJobs(context.Background(), &commonpb.Empty{})
	if err != nil {
		con.PrintErrorf("%s\n", err)
		return
