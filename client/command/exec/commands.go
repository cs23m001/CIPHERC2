...
func Commands(con *console.CIPHERC2Client) []*cobra.Command {
	...
	executeShellcodeCmd := &cobra.Command{
		...
		Short: "Executes the given shellcode in the CIPHERC2 process",
	...
	psExecCmd := &cobra.Command{
		...
		Short: "Start a CIPHERC2 service on a remote target",
	...
	flags.Bind("", false, psExecCmd, func(f *pflag.FlagSet) {
		f.StringP("service-name", "s", "CIPHERC2", "name that will be used to register the service")
		f.StringP("service-description", "d", "CIPHERC2 implant", "description of the service")
		...
		f.StringP("custom-exe", "c", "", "custom service executable to use instead of generating a new CIPHERC2")
