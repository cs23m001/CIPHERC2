...
func SSHCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	var (
		privKey []byte
		err     error
	)
	...
	sshCmd, err := con.Rpc.RunSSHCommand(context.Background(), &CIPHERC2pb.SSHCommandReq{
		Username: username,
		Hostname: hostname,
		Port:     uint32(port),
		PrivKey:  privKey,
...
func PrintSSHCmd(sshCmd *CIPHERC2pb.SSHCommand, con *console.CIPHERC2Client) {
...
func tryCredsFromLoot(con *console.CIPHERC2Client) (string, string, []byte) {
	var (
		username string
		password string
		privKey  []byte
