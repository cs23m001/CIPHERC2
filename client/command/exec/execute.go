...
func ExecuteCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	if token || hidden || ppid != 0 {
		...
		exec, err = con.Rpc.ExecuteWindows(context.Background(), &CIPHERC2pb.ExecuteWindowsReq{
		...
		exec, err = con.Rpc.Execute(context.Background(), &CIPHERC2pb.ExecuteReq{
			Request: con.ActiveTarget.Request(cmd),
			Path:    cmdPath,
			Args:    args,
			Output:  captureOutput,
...
func HandleExecuteResponse(exec *CIPHERC2pb.Execute, cmdPath string, hostName string, cmd *cobra.Command, con *console.CIPHERC2Client) {
...
func PrintExecute(exec *CIPHERC2pb.Execute, cmd *cobra.Command, con *console.CIPHERC2Client) {
	ignoreStderr, _ := cmd.Flags().GetBool("ignore-stderr")
	stdout, _ := cmd.Flags().GetString("stdout")
	stderr, _ := cmd.Flags().GetString("stderr")
...
func combineCommandOutput(exec *CIPHERC2pb.Execute, combineStdOut bool, combineStdErr bool) []byte {
...
func LootExecute(commandOutput []byte, lootName string, CIPHERC2CmdName string, cmdName string, hostName string, con *console.CIPHERC2Client) {
	...
	fileName := fmt.Sprintf("%s_%s_%s_%s.log", CIPHERC2CmdName, hostName, shortCommandName, timeNow)
	if lootName == "" {
		lootName = fmt.Sprintf("[%s] %s on %s (%s)", CIPHERC2CmdName, shortCommandName, hostName, timeNow)
	}
...
func PrintExecutionOutput(executionOutput string, saveOutput bool, commandName string, hostName string, con *console.CIPHERC2Client) {
	con.PrintInfof("Output:\n%s", executionOutput)
...
func SaveExecutionOutput(executionOutput string, commandName string, hostName string, con *console.CIPHERC2Client) {
