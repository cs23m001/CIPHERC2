...
func StartTCPListenerCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	listener, err := con.Rpc.PivotStartListener(context.Background(), &CIPHERC2pb.PivotStartListenerReq{
		Type:        CIPHERC2pb.PivotType_TCP,
...
func StartNamedPipeListenerCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	session := con.ActiveTarget.GetSessionInteractive()
	if session == nil {
		return
	}
	...
	listener, err := con.Rpc.PivotStartListener(context.Background(), &CIPHERC2pb.PivotStartListenerReq{
		Type:        CIPHERC2pb.PivotType_NamedPipe,
		BindAddress: bind,
		Request:     con.ActiveTarget.Request(cmd),
		Options:     options,
	})
