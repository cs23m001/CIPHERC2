...
func ShellCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	if con.ActiveTarget.GetSession().OS != linux && con.ActiveTarget.GetSession().OS != darwin {
		noPty = true // CIPHERC2's PTYs are only supported on linux/darwin
	}
	runInteractive(cmd, shellPath, noPty, con)
	con.Println("Shell exited")
}

func runInteractive(cmd *cobra.Command, shellPath string, noPty bool, con *console.CIPHERC2Client) {
	con.Println()
