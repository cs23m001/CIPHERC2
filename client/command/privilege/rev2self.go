...
func RevToSelfCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}

	revert, err := con.Rpc.RevToSelf(context.Background(), &CIPHERC2pb.RevToSelfReq{
...
func PrintRev2Self(revert *CIPHERC2pb.RevToSelf, con *console.CIPHERC2Client) {
