...
func ExtensionLoadCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	dirPath := args[0]
	...
	CIPHERC2Menu := con.App.Menu("implant")
	for _, extCmd := range manifest.ExtCommand {
		if CmdExists(extCmd.CommandName, CIPHERC2Menu.Command) {
...
func ExtensionRegisterCommand(extCmd *ExtCommand, cmd *cobra.Command, con *console.CIPHERC2Client) {
...
func loadExtension(goos string, goarch string, checkCache bool, ext *ExtCommand, cmd *cobra.Command, con *console.CIPHERC2Client) error {
	...
	if checkCache {
		extList, err := con.Rpc.ListExtensions(context.Background(), &CIPHERC2pb.ListExtensionsReq{
...
func registerExtension(goos string, ext *ExtCommand, binData []byte, cmd *cobra.Command, con *console.CIPHERC2Client) error {
	...
	if beac != nil && sess == nil {
		go func() {
			registerResp, err := con.Rpc.RegisterExtension(context.Background(), &CIPHERC2pb.RegisterExtensionReq{
				Name:    name,
				Data:    binData,
				OS:      goos,
				Init:    ext.Init,
...
func loadDep(goos string, goarch string, depName string, cmd *cobra.Command, con *console.CIPHERC2Client) error {
...
func runExtensionCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	var (
		err           error
		extensionArgs []byte
		extName       string
	...
	callExtResp, err := con.Rpc.CallExtension(context.Background(), &CIPHERC2pb.CallExtensionReq{
		Name:    name,
		Export:  entryPoint,
		Args:    extensionArgs,
		Request: con.ActiveTarget.Request(cmd),
...
func PrintExtOutput(extName string, commandName string, outputSchema *packages.OutputSchema, callExtension *CIPHERC2pb.CallExtension, con *console.CIPHERC2Client) {
