...
func ExtensionsRemoveCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	name := args[0]
	if name == "" {
...
func RemoveExtensionByCommandName(commandName string, con *console.CIPHERC2Client) error {
	if commandName == "" {
...
func RemoveExtensionByManifestName(manifestName string, con *console.CIPHERC2Client) (bool, error) {
	if manifestName == "" {
