...
func CrackCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func CrackStationsCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	crackers, err := con.Rpc.Crackstations(context.Background(), &commonpb.Empty{})
	if err != nil {
		con.PrintErrorf("%s\n", err)
		return
...
func PrintNoCrackstations(con *console.CIPHERC2Client) {
	con.PrintInfof("No crackstations connected to server\n")
}

func AreCrackersOnline(con *console.CIPHERC2Client) bool {
	crackers, err := con.Rpc.Crackstations(context.Background(), &commonpb.Empty{})
	if err != nil {
		return false
	}
...
func PrintCrackers(crackers []*clientpb.Crackstation, con *console.CIPHERC2Client) {
...
func printCracker(cracker *clientpb.Crackstation, index int, con *console.CIPHERC2Client) {
	tw := table.NewWriter()
...
func printBenchmarks(cracker *clientpb.Crackstation, con *console.CIPHERC2Client) {
	tw := table.NewWriter()
	tw.SetStyle(settings.GetTableStyle(con))
