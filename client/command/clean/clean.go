...
// CleanCmd - Remove all profiles, beacons, sessions and implant builds (Builds and logs will still exist on disk in .CIPHERC2)
func CleanCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func removeImplantBuilds(con *console.CIPHERC2Client) error {
...
func removeProfiles(con *console.CIPHERC2Client) error {
...
func removeSessionsAndBeacons(con *console.CIPHERC2Client) error {
	...
	for _, session := range sessions.Sessions {
		_, err := con.Rpc.Kill(context.Background(), &CIPHERC2pb.KillReq{
...
func Command(con *console.CIPHERC2Client) []*cobra.Command {
	return []*cobra.Command{{
		Use:   "clean",
		Short: "Remove all profiles, beacons, sessions, implant builds and HTTP profiles (Builds and logs will still exist on disk in .CIPHERC2)",
