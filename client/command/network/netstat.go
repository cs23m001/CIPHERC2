...
func NetstatCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	netstat, err := con.Rpc.Netstat(context.Background(), &CIPHERC2pb.NetstatReq{
...
func PrintNetstat(netstat *CIPHERC2pb.Netstat, implantPID int32, activeC2 string, numeric bool, con *console.CIPHERC2Client) {
	lookup := func(skaddr *CIPHERC2pb.SockTabEntry_SockAddr) string {
		addr := skaddr.Ip
