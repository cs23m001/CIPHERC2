...
*/
func PerformDownload(remotePath string, fileName string, cmd *cobra.Command, con *console.CIPHERC2Client) (*CIPHERC2pb.Download, error) {
	ctrl := make(chan bool)
	con.SpinUntil(fmt.Sprintf("%s -> %s", fileName, "loot"), ctrl)
	download, err := con.Rpc.Download(context.Background(), &CIPHERC2pb.DownloadReq{
...
func SendLootMessage(loot *clientpb.Loot, con *console.CIPHERC2Client) {
...
func LootDownload(download *CIPHERC2pb.Download, lootName string, fileType clientpb.FileType, cmd *cobra.Command, con *console.CIPHERC2Client) {
...
func LootText(text string, lootName string, lootFileName string, fileType clientpb.FileType, con *console.CIPHERC2Client) {
	lootMessage := CreateLootMessage(con.ActiveTarget.GetHostUUID(), lootFileName, lootName, fileType, []byte(text))
	SendLootMessage(lootMessage, con)
}

func LootBinary(data []byte, lootName string, lootFileName string, fileType clientpb.FileType, con *console.CIPHERC2Client) {
...
func LootAddRemoteCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
