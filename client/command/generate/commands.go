...
// coreImplantFlags binds all flags common to all CIPHERC2 implant types.
// This is used by all CIPHERC2 compilation and profiles generation commands.
...
func coreImplantFlagCompletions(cmd *cobra.Command, con *console.CIPHERC2Client) {
...
func compileImplantFlags(name string, cmd *cobra.Command) {
	flags.Bind(name, false, cmd, func(f *pflag.FlagSet) {
		f.StringP("name", "N", "", "agent name")
		f.StringP("template", "I", "CIPHERC2", "implant code template")
