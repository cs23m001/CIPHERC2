...
func ImplantsCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
...
func PrintImplantBuilds(builds *clientpb.ImplantBuilds, filters ImplantBuildFilter, con *console.CIPHERC2Client) {
	tw := table.NewWriter()
	...
	for CIPHERC2Name, config := range builds.Configs {
		...
		implantType := ""
		if builds.ResourceIDs[CIPHERC2Name].Type != "" {
			implantType = builds.ResourceIDs[CIPHERC2Name].Type
		}
		...
		tw.AppendRow(table.Row{
			...
			fmt.Sprintf("%v", builds.ResourceIDs[CIPHERC2Name].Value),
			fmt.Sprintf("%v", builds.Staged[CIPHERC2Name]),
		})
	}
...
func ImplantBuildNameCompleter(con *console.CIPHERC2Client) carapace.Action {
...
func ImplantBuildByName(name string, con *console.CIPHERC2Client) *clientpb.ImplantConfig {
	builds, err := con.Rpc.ImplantBuilds(context.Background(), &commonpb.Empty{})
	if err != nil {
		return nil
	}
	for CIPHERC2Name, build := range builds.Configs {
		if CIPHERC2Name == name {
			return build
		}
	}
	return nil
