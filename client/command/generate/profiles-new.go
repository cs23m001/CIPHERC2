...
func ProfilesNewCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	var name string
	if len(args) > 0 {
		name = args[0]
	}
...
func ProfilesNewBeaconCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	var name string
	if len(args) > 0 {
		name = args[0]
	}
