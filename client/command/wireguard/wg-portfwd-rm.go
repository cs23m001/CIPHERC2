...
func WGPortFwdRmCmd(cmd *cobra.Command, con *console.CIPHERC2Client, args []string) {
	...
	stopReq, err := con.Rpc.WGStopPortForward(context.Background(), &CIPHERC2pb.WGPortForwardStopReq{
		ID:      int32(fwdID),
...
func PortfwdIDCompleter(con *console.CIPHERC2Client) carapace.Action {
	callback := func(_ carapace.Context) carapace.Action {
		...
		fwdList, err := con.Rpc.WGListForwarders(context.Background(), &CIPHERC2pb.WGTCPForwardersReq{
