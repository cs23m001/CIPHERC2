...
const (
	// CIPHERC2ClientDirName - Directory storing all of the client configs/logs
	CIPHERC2ClientDirName = ".CIPHERC2-client"

	versionFileName = "version"
)

// GetRootAppDir - Get the CIPHERC2 app dir ~/.CIPHERC2-client/
func GetRootAppDir() string {
	user, _ := user.Current()
	dir := filepath.Join(user.HomeDir, CIPHERC2ClientDirName)
...
// GetClientLogsDir - Get the CIPHERC2 client logs dir ~/.CIPHERC2-client/logs/
...
// GetConsoleLogsDir - Get the CIPHERC2 client console logs dir ~/.CIPHERC2-client/logs/console/
...
func Setup(force bool, echo bool) {
	...
	if force || localVer == "" || localVer != ver.GitCommit {
		if echo {
			fmt.Printf(`
CIPHERC2  Copyright (C) 2022  Bishop Fox
