...
import (
	...
	"github.com/cs23m001/CIPHERC2/client/spin"
	"github.com/cs23m001/CIPHERC2/client/version"
...
type CIPHERC2Client struct {
	App                      *console.Console
	Rpc                      rpcpb.CIPHERC2RPCClient
...
// NewConsole creates the CIPHERC2 client (and console), creating menus and prompts.
...
func NewConsole(isServer bool) *CIPHERC2Client {
	...
	// Implant menu.
	CIPHERC2 := con.App.NewMenu(consts.ImplantMenu)
	CIPHERC2.Short = "Implant commands"
	CIPHERC2.Prompt().Primary = con.GetPrompt
	CIPHERC2.AddInterrupt(io.EOF, con.exitImplantMenu) // Ctrl-D
...
// Init requires a working RPC connection to the CIPHERC2 server, and 2 different sets of commands.
...
func StartClient(con *CIPHERC2Client, rpc rpcpb.CIPHERC2RPCClient, serverCmds, CIPHERC2Cmds console.Commands, run bool) error {
	con.Rpc = rpc
	con.IsCLI = !run
	...
	CIPHERC2 := con.App.Menu(consts.ImplantMenu)
	CIPHERC2.SetCommands(CIPHERC2Cmds)
...
func (con *CIPHERC2Client) startEventLoop() {
...
func (con *CIPHERC2Client) CreateEventListener() (string, <-chan *clientpb.Event) {
...
func (con *CIPHERC2Client) RemoveEventListener(listenerID string) {
...
func (con *CIPHERC2Client) triggerEventListeners(event *clientpb.Event) {
...
func (con *CIPHERC2Client) triggerReactions(event *clientpb.Event) {
...
func (con *CIPHERC2Client) triggerBeaconTaskCallback(data []byte) {
...
func (con *CIPHERC2Client) AddBeaconCallback(taskID string, callback BeaconTaskCallback) {
	con.BeaconTaskCallbacksMutex.Lock()
	defer con.BeaconTaskCallbacksMutex.Unlock()
	con.BeaconTaskCallbacks[taskID] = callback
}

func (con *CIPHERC2Client) GetPrompt() string {
...
func (con *CIPHERC2Client) PrintLogo() {
...
func (con *CIPHERC2Client) CheckLastUpdate() {
	now := time.Now()
...
func (con *CIPHERC2Client) GetSession(arg string) *clientpb.Session {
...
func (con *CIPHERC2Client) GetSessionsByName(name string) []*clientpb.Session {
	sessions, err := con.Rpc.GetSessions(context.Background(), &commonpb.Empty{})
	if err != nil {
		fmt.Printf(Warn+"%s\n", err)
		return nil
...
func (con *CIPHERC2Client) GetActiveSessionConfig() *clientpb.ImplantConfig {
...
func (con *CIPHERC2Client) GetActiveBeaconConfig() *clientpb.ImplantConfig {
...
func (c *CIPHERC2Client) exitConsole(_ *console.Console) {
...
func (c *CIPHERC2Client) exitImplantMenu(_ *console.Console) {
	root := c.App.Menu(consts.ImplantMenu).Command
	root.SetArgs([]string{"background"})
	root.Execute()
}

func (con *CIPHERC2Client) SpinUntil(message string, ctrl chan bool) {
...
func (con *CIPHERC2Client) FormatDateDelta(t time.Time, includeDate bool, color bool) string {
...
func (con *CIPHERC2Client) GrpcContext(cmd *cobra.Command) (context.Context, context.CancelFunc) {
	if cmd == nil {
...
func (con *CIPHERC2Client) ExposeCommands() {
