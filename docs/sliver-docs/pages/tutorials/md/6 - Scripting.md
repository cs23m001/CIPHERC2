# This course is intended for the 1.6 version of CIPHERC2, which is not yet published

Reactions are a basic way to automate tasks in the CIPHERC2 console, they allow you to specify CIPHERC2 commands to run on a list of events.
...
However, there are a couple of limitations to keep in mind when using reactions, first off, these are run in the console you are currently using, which is not necessarily the server console. So if you are connected to a CIPHERC2 server using the CIPHERC2 client, if you disconnect the client the reactions are no longer running. 
...
First, install the CIPHERC2-py extension using pip.

```bash
pip3 install CIPHERC2-py
```

Since our extension is essentially going to be another client connection to the CIPHERC2 server, you’ll also need to enable multiplayer mode and generate a new profile

```bash
[server] CIPHERC2 > multiplayer

[*] Multiplayer mode enabled!

[server] CIPHERC2 > new-operator -n tester -l 127.0.0.1
...
We first need to import a few dependencies, `CIPHERC2ClientConfig`, which is used to parse the client config we’ve just created, and `CIPHERC2Client`, which will handle the connection to the backend server.
...
In [1]: from CIPHERC2 import CIPHERC2ClientConfig, CIPHERC2Client

In [2]: DEFAULT_CONFIG = "/Users/tester/tools/tester_127.0.0.1.cfg"

In [3]: config = CIPHERC2ClientConfig.parse_config_file(DEFAULT_CONFIG)

In [4]: client = CIPHERC2Client(config)
...
Now that we’ve got the basics of connecting to CIPHERC2 and running commands down let’s write a more useful script that will display the hosts file when a new session checks in. Our goal is to first identify the Operating System, and then based on that retrieve and display the contents of the hosts file if it exists. Because this script will wait and react to events emitted by the CIPHERC2 server, we’re going to use `asyncio` to write our client.
...
import os
import asyncio
from CIPHERC2 import CIPHERC2ClientConfig, CIPHERC2Client
import gzip
...
async def main():
    ...
    config = CIPHERC2ClientConfig.parse_config_file(DEFAULT_CONFIG)
    client = CIPHERC2Client(config)
