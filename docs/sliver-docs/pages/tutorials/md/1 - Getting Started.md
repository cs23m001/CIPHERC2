...
`CIPHERC2-server` is the binary you want to use to run the CIPHERC2 C2 server, `CIPHERC2-client` is solely a client to connect to a CIPHERC2 C2 server. CIPHERC2 server also acts as a client on its own, so you don’t necessarily run CIPHERC2 server and client separately.

First time running CIPHERC2 will take a couple seconds as it's retrieving its dependencies. Consecutive executions will be much faster. Go ahead and launch the `CIPHERC2-server`.

```asciinema
{"src": "/asciinema/startup.cast", "cols": "132", "rows": "28", "idleTimeLimit": 8}
```

Let's take a couple minutes to discuss what CIPHERC2 actually is and how it's set up.

![Alt text](/images/Architecture.png)

Now that CIPHERC2 is running, lets generate and execute your first implant to try out some of the basic features of CIPHERC2, for now we’re going to run everything on the local host.
...
[server] CIPHERC2 (RELATED_EARDRUM) > pwd
