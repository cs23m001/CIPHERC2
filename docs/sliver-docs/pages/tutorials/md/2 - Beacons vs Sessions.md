...
CIPHERC2 implants support two types of connections, sessions and beacons.
...
```bash
[server] CIPHERC2 (TIRED_GIRAFFE) > reconfig -i 5s -j 1s
...
[server] CIPHERC2 (TIRED_GIRAFFE) > info
...
[server] CIPHERC2 (TIRED_GIRAFFE) > use
...
[server] CIPHERC2 (TIRED_GIRAFFE) > socks5 start
