...
When using CIPHERC2 during a live engagement, you’re going to need to use custom stagers, which are essentially a first binary or commandline that will retrieve and/or load CIPHERC2 into memory on your target system. CIPHERC2 can generate shellcode for your stager to execute by using the `profiles` command.
...
```
https://CIPHERC2-ip/whatever.stager_file_ext?x=yourID
```
...
CIPHERC2 staging also supports encoding or encrypting our payloads before exposing them externally using the `profile stage` command, the implant configuration remains the same but you are now able to stage different versions of it simultaneously.
