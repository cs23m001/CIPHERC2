...
The CIPHERC2 armory is used to install and maintain third party extensions and aliases within CIPHERC2. The full list of available extensions can be found at https://github.com/CIPHERC2armory/armory, keep in mind this is community maintained so not all modules are necessarily up to date. 

You can download all configured extensions/aliases using the armory command.

```bash
[server] CIPHERC2 > armory install all
...
```bash
[server] CIPHERC2 (UNABLE_PRIDE) > seatbelt " WindowsCredentialFiles"
...
As you can see CIPHERC2 ran the Seatbelt assembly and provided us with the output of our command.

## Bof’s

Beacon object files are loaded using trustedsec’s coffloader. When you run a bof command the loader will first be loaded into memory and is used to run whichever bof you choose. From an operator’s perspective bof’s are similar to basic CIPHERC2 commands.

```bash
[server] CIPHERC2 (UNABLE_PRIDE) > sa-whoami
